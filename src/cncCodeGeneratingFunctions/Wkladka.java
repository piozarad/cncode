...
import static CordConverter.Function.*;
...
public class Wkladka extends JFrame implements ActionListener {
	...
	public Wkladka(Edytor parent)
	{
		...
		border.gridx=0;
		border.gridy++;
		startLabel = new JLabel("Z poczatkowe");
		startLabel.setToolTipText("Z do ktorego podjezdza narzedzie przed rozpoczeciem pracy. Wspolrzedna nalezy ustawic przed materialem");
		...
		endLabel.setToolTipText("Wspolrzedna Z dna frezowanej wkladki");
		add(endLabel,border);
		...
		border.gridx=0;
		border.gridy++;
		toolDiameterLabel = new JLabel("srednica narzedzia");
		toolDiameterLabel.setToolTipText("srednica narzedzia wykorzystanego do operacji podana w mm");
		...
		calculate.setToolTipText("Generuj program dla podanych parametrow");
		...
		speedUnit.setToolTipText("obroty na minute");
		...
		//x2 y9 feed
 		...
 		feedUnit.setToolTipText("milimetry na minute");
 		add(feedUnit,border);
	...
	private boolean isOk()
	{
		...
		{
			JOptionPane.showMessageDialog(this, "zle podane ae");
			return false;
		}
		try
		{
		...
		if(speed <0)
		{
			JOptionPane.showMessageDialog(this, "Predkosœc wrzeciona nie moze byc ujemna");
			return false;
		}
		if(feed <0)
		{
			JOptionPane.showMessageDialog(this, "Predkosc posuwu nie moze byc ujemna");
			return false;
		}
		if(ae <=0 || ae>0.601)
		{
		...
		if(toolDiameter <0)
		{
			JOptionPane.showMessageDialog(this, "srednica narzedzia nie moze byc ujemna");
			return false;
		}
		try
		{
	...
	private void generate()
	{
		...
		if(this.parent.getControls().isType(ControlTypes.SINUMERIC))
		...
		parent.writelog("Wygenerowano kod dla rozfrezowania okraglej wkladki o srednicy "+diameter+"mm na wspolrzednych X"+x+ " Y"+y+ "\n\tsrednica uzytego narzedzia: "+toolDiameter+"mm. Mozliwa korekcja promieniowa do 2mm ");
	}
