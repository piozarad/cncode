...
public class wiercenie extends JFrame {

	Edytor parent;
	 static final String []types = {"G81 Wiercenie proste", "G82 Pog³êbienie/nawiercenie", "G83 Wiercenie g³ebokie", "G76/G1 Wytaczanie","G84 Gwintowanie" ,"G85 Rozwiercanie"};
	...
	public wiercenie(Edytor parent)
	{
		...
		//zsz
		border.gridy++;
		katPromienXY = new JButton("Wspo³rzêdne");
		katPromienXY.setToolTipText("Definiuj po³o¿enie przez wspó³rzêdne lub k¹t i promieñ");
		...
		if(angleRadius)
		{
			...
			katPromienXY.setText("Wspó³rzêdne");
			angleRadius=false;
		}
		else
		{
			xWierceniaLabel.setText("Promieñ");
			xWierceniaLabel.setToolTipText("Promieñ od œrodka uk³adu wspó³rzêdnych do osi otworu");
		...
		zPodjazdLabel.setToolTipText("<html> Wspolrzedna z do ktorej narzedzie podjezdza ruchem szybkim i zaczyna wierenie z zadanym posuwem. <br>Wspolrzedna musi znajdowac sie poza materia³em </html>");
		...
		dodatkowyParametrLabel.setToolTipText("Parametr zale¿ny od wybranego cyklu ");
		...
		oblicz.addActionListener(e->{
			...
			{
				drillTwoSides();
				parent.writelog("Wygenerowano cykl sta³y dla g³ebokiego otworu - wiercenie z obu stron");
				this.cofnij.doClick();
			}
			else{
				if(wiecejOtworow)
				{
					if(getValues())
					{
						drillMultipleHoles();
						this.cofnij.doClick();
						parent.writelog("Wygenerowano cykl sta³y dla grupy otworów");
					}
				...
				else
				{
					...
					{
						drill();
						parent.writelog("Wygenerowano cykl sta³y");
						this.cofnij.doClick();
					}
				}
				}

			Wind.log.writeInfoLog("wiercenie - wykonano", wiercenie.class.getSimpleName());
		...
		typeCombo.addActionListener(e->{
			...
			{
				dodatkowyParametrLabel.setText("Dodatkowy parametr");
				dodatkowyParametrLabel.setToolTipText("<html>Cykl G82 - brak dodatkowych paremetrow<br><Postój czasowy na dnie otworu liczony jest automatycznie dla 1.5 obrotu narzêdzia na dnie otworu</html>");
				dodatkowyParametrTxt.setText("");
				dodatkowyParametrTxt.setEditable(false);
			...
			{
				dodatkowyParametrLabel.setText("G³êbokoœsc zag³êbiania Q");
				dodatkowyParametrLabel.setToolTipText("Wiert³o bêdziê siê zag³êbia³o o podan¹ wartoœæ za ka¿dym razem nastêpnie wyje¿dza³o do punktu pocz¹tkowego przed materia³ a¿ do zakoñczenia wiercenia");
			...
			else if(i==3)
			{
				...
				dodatkowyParametrLabel.setToolTipText("<html>Na dnie otworu wrzeciono zatrzymuje siê i odsuwa o podan¹ wartoœæ aby nie rysowaæ materia³u podczas wyjazdu <br>Kierunek odjazdu zale¿y od wybranego sterowania, dla Fanuca jest to -Y dla Sinumerica X+ </html>");
				dodatkowyParametrTxt.setEditable(true);
				wiercZDwochStron.setEnabled(false);
			}
			else if(i==4)
		...
		dodajWiecejOtworow.setToolTipText("Dodaj wiêcej otworow do cyklu");
		...
		bezpiecznyOdjazdRadio.setToolTipText("Podczas wiercenia wiele otworów ustawia przejazd miêdzy otworami na Z podjazdu +200");
		add(bezpiecznyOdjazdRadio,border);
		
		//wierc z 2 stron
		border.gridy=8;
		wiercZDwochStron = new JRadioButton("Wiecenie z dwóch stron");
		wiercZDwochStron.setToolTipText("Dzieli g³ebokie wiercenie na dwie strony obracaj¹c stó³ o 180 stopni");
		wiercZDwochStron.setEnabled(true);
...
//				System.out.printf(Locale.CANADA,"N%d G1 Z%.2f F5000. %n",n+=5,r);
//				System.out.printf(Locale.CANADA,"N%d G1 Z%.2f F%.0f %n",n+=5,z,feed);
//				System.out.printf(Locale.CANADA,"N%d M5",n+=5);
//				System.out.printf(Locale.CANADA,"N%d G4 X1%n",n+=5);
//				System.out.printf(Locale.CANADA,"N%d M19%n",n+=5);
//				System.out.printf(Locale.CANADA,"N%d G4 X1%n",n+=5);
//				System.out.printf(Locale.CANADA,"N%d G0 X%.2f%n",n+=5,x+0.1);
...
//				break;
	...
	private boolean getPoint()
	{
		...
		{
			JOptionPane.showMessageDialog(this, "Zle zdefiniowana wspó³rzêdna x", "Blad", JOptionPane.NO_OPTION);
			return false;
		}
		try
		{
		...
		{
			JOptionPane.showMessageDialog(this, "Zle zdefiniowana wspó³rzêdna y", "Blad", JOptionPane.NO_OPTION);
			return false;
		}
	...
	private boolean getValues()
	{
		...
		{
			JOptionPane.showMessageDialog(this, "Zle zdefiniowana prêdkoœæ wrzeciona", "Blad", JOptionPane.NO_OPTION);
			return false;
		}
		try
		{
		...
		{
			JOptionPane.showMessageDialog(this, "Zle zdefiniowana prêdkoœæ posuwu", "Blad", JOptionPane.NO_OPTION);
			return false;
		}
		...
		{
			JOptionPane.showMessageDialog(this, "Zle zdefiniowana wspó³rzêdna dna otworu", "Blad", JOptionPane.NO_OPTION);
			return false;
		}
		try
		{
		...
		if(r<=z)
		{
			JOptionPane.showMessageDialog(this, "Podjazd zbyt ma³y w stosunku do wspó³rzednej Z wiercenia", "Ju¿ byœ przydzwoni³", JOptionPane.NO_OPTION);
			return false;
		}
	...
	class Display extends JFrame
	{
		...
		DefaultListModel<Point> cloneModel()
		{
			...
			for(int i=0; i< wiercenie.this.model.size(); i++)
			{
				result.addElement(wiercenie.this.model.getElementAt(i));
			}
			return result;
		}
